 ...
 * @author Joshua
 */
public class JIF_Lista_Cargos extends javax.swing.JInternalFrame {
    ...
    {   try
        {   st=con.createStatement();
            rs=st.executeQuery("SELECT T.`CODIGO`, CONCAT(P.`APELLIDOP`, ' ',P.`APELLIDOM`, ' ',P.`NOMBRES`), A.`AREA` FROM Trabajador T INNER JOIN Persona P ON T.`DNI`=P.`DNI` INNER JOIN AREA A ON T.`IDAREA`=A.`ID` ORDER BY T.`CODIGO`");
    ...
    private void initComponents() {
        ...
        setIconifiable(true);
        setMaximizable(true);
        setTitle("Lista de Cargos - Consorcio Coopsol Megaelectric");
...
//        }
    ...
    private void jTextField1CaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_jTextField1CaretUpdate
        try
        {   st=con.createStatement();
            rs=st.executeQuery("SELECT T.`CODIGO`, CONCAT(P.`APELLIDOP`, ' ',P.`APELLIDOM`, ' ',P.`NOMBRES`), A.`AREA` FROM Trabajador T INNER JOIN Persona P ON T.`DNI`=P.`DNI` INNER JOIN AREA A ON T.`IDAREA`=A.`ID` WHERE CONCAT(P.`APELLIDOP`, ' ',P.`APELLIDOM`, ' ',P.`NOMBRES`) LIKE '%"+jTextField1.getText()+"%' ORDER BY T.`CODIGO`;");
            modelo=(DefaultTableModel) jTable1.getModel();
            limpiarTabla();
            while(rs.next())
            {   Object rowData[]={rs.getString(1), rs.getString(2), rs.getString(3)};
